...
Correlation: how tied the variables are with 1 and -1 strong correlations and values close to 0 are weak. If positive then positive relationship and if not then inverse relationship. Autocorrelation: how your series is related to other observations of the same series. Calculate by creating new column with a lag of 1 from actual column then can lag by 2. info on self-correlation and time dependency. Partial autocorrelation: y and yt-2 there is some influence of yt-1 in the middle. gives relationship of the y and yt-2 when the influence of intermediates are eliminated. 

de-trending: want to create an equation that will model the data i have. trend is one example. when model and want to think of future trends then want the trend to be in our future projections. some models can't handle trends so model then put trend back in. most models can handle the trend though.
...
Need to clean the dataset within R before starting. 
...
* **Mann-Kendall**: no seasonality, non-parametric, missing data allowed. Function: `MannKendall()` (package: Kendall). stationary test to see if the change is really ocurring. may make blind on trend if not account for season
* **Seasonal Mann-Kendall**: seasonality, non-parametric `SeasonalMannKendall` (package: Kendall)
* **Spearman Rho**: no seasonality, non-parametric, missing data allowed. Function: `cor.test(method="spearman")` (package: stats), correlation of time series overtime. see if increase or decrease. cannot handle seasonality

Specific test for stochastic trend analysis:

* **Augmented Dickey Fuller**: no seasonality, non-parametric, missing data not allowed. Function: `adf.test()` (package: tseries), detect stochastic trend. stochastic trend is trend that's slope changes overtime
determinisitic is trend that is constant over time
...
This section is a where to start on time series for forecasting beyond this lesson. 
