...
use the decision tree on data-to-viz.com to decide
...
### Aesthetics, if plotting one variable can plot aes at top in general ggplot code section, if plotting many variables than define aes for each y-variable
...
# Scatterplot, dataframe is always in the first ggplot call line. has to be a dataframe, not vector or times series. 
...
#Luana's preference is always to filter before getting into plot but if it's a little thing than you can filter in it
PMplot.faceted2 <- 
...
# Violin plot, shows distribution of data
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
