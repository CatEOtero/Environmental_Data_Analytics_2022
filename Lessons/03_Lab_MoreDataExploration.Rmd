...
* **Data Frames** 2 dimensional structure that is more general than a matrix. Columns can have different modes (e.g., numeric and factor). When we import csv files into the R workspace, they will enter as data frames. A lot more freedom but they columns have to be the same length so fill in empty spots with NAs
...
```{r}
# Vectors ---- vectors are stored as values in the environment
vector1 <- c(1,2,5.3,6,-2,4) # numeric vector, can have decimals and negatives
vector1
vector2 <- c("one","two","three") # character vector
vector2
vector3 <- c(TRUE,TRUE,TRUE,FALSE,TRUE,FALSE) #logical vector, remember capitals
vector3

vector1[3] # 

# Matrices ---- always specify the number of rows and columns
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # 
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #makes the rows fill first then columns
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line, dimnames is how you add names to rows and columns
                  dimnames = list(c("uno", "dos", "tres", "cuatro", "cinco"), 
                                  c("un", "deux", "trois", "cat"))) #
matrix3

matrix1[4, ] #prints row 4
matrix1[ , 3] #prints column 3
matrix1[c(12, 14)] #prints value in row 12, column 14???
matrix1[c(12:14)] #?????
matrix1[2:4, 1:3] #print row 2 thru 4 and column 1 thru 3
...
Question: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens? Sometimes don't even need to press tab. Tab will let R know you need help finding the name of something

> Answer: R brings up a list of element options from the Dataframe
...
```{r}
HighTemps <- c(49, 57, 40, 41, 44, 37, 41, 51, 52, 52)

LowTemps <- c(30, 30, 18, 26, 26, 26, 18, 28, 28, 40)
```
...
```{r}
FarenToCelcius <- function(x){
  convert <- (x-32)*(5/9)
  return(convert)
  }

HighTemps_c <- FarenToCelcius(HighTemps)

LowTemps_c <- FarenToCelcius(LowTemps)
LowTemps_c

#to get rid of the long numbers after decimals you can use the round() function
round(34.25678, digits = 2)
#output is 34.26
```
...
```{r}
DurhamForecast <- data.frame(HighTemps, HighTemps_c, LowTemps, LowTemps_c)

DurhamForecast

#Figure out how to name the columns in the one line when creating a dataframe. and not use following command,not updated for the temp code yet
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # 
```

Use the common functions `summary` and `sd` to obtain basic data summaries of the ten-day forecast. How would you call these functions differently for the entire data frame vs. a single column? Attempt to demonstrate both options below. actually sd only works on vectors not dataframes!
...
Sidenote: when you use the function print() instead of just writing out and running your object by name then you can add text in front of the numbers and other stuff you run. You can fancify your ouputs. 
...
Important to remember the formatting so you can interpret dates in any form and have R be able to read them as dates
