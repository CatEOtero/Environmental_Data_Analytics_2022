...
#Exercise 2: Tidy the rainfall data sets. ASK ABOUT IN OH
...
#separate function: split one column into multiple
#seperate(columnname, c("newcolname", "newcolname", "newcolname")" ")
#the nulls in this case is column names of our new columns and we won't use the null one
#the space btw the last set of "" is telling R that a space seperates what we want to divide the columns to divide into i.e. "Jan Average" is divided by a space
#paste0 pastes all the elements together
...
#Exercise 4: Join the temperature and rainfall into one tidy data frame with 3 columns: Date, Rainfall and Temperature. 10:55 during lesson - why ue the innerjoin
...
Temp <- ggplot(BRA_complete, aes(x = Date, y = Temperature_C)) +
  geom_line() +
  geom_smooth(method = "lm")


Rain <- ggplot(BRA_complete, aes(x = Date, y = Rainfall)) +
  geom_line() +
  geom_smooth(method = "lm")
...
```{r}
#Exercise 7: Create a time series objects for each series using ts(). Make sure you specify the starting point and frequency. Make sure you point out the column in the dataset that you want converted to a time series or else R will try to do all of the data. 
f_month_rain <- month(first(Rainfall_BR_processed$Date))
f_year_rain <- year(first(Rainfall_BR_processed$Date))
rain_data_ts <- ts(Rainfall_BR_processed$Rainfall,
                   start=c(f_year_rain, f_month_rain),
                   frequency=12)


f_month_temp <- month(first(Temp_BR_processed$Date))
f_year_temp <- year(first(Temp_BR_processed$Date))
temp_data_ts <- ts(Temp_BR_processed$Temperature_C,
                   start=c(f_year_temp, f_month_temp),
                   frequency=12)
#freq is 12 b/c we are doing it by month
#or
#11:25
...
You can see how much is due to each part
...
# Visualize the decomposed series using plot(). What components can you identify? stl shows seasonal trend and remainders. first windeow is original data, second is seasonal componnent, third is trend and fourth is remainder. if you some the bottom three you get your original data

rain_data_decomp <- stl(rain_data_ts,s.window = "periodic")
plot(rain_data_decomp)

temp_data_decomp <- stl(temp_data_ts,s.window = "periodic")
plot(temp_data_decomp)


# Exercise 9: Use the times series object generated with the stl() to extract the 
# components of each series and turn them into data frames. If having a hard time finding the time.series then look at the environment
