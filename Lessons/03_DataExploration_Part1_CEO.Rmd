...
A session in RStudio will always function by mapping to a specific folder in your computer, called the *working directory*. All navigation between folders and files will happen relative to this working directory. When you open an R project, your working directory will automatically set to the folder that holds the project file. If you open an R script or RMarkdown document directly by double-clicking the file, your working directory will automatically set to the folder that holds that file. 

SMART: It is a good idea to note with a comment at the top of your file which working directory you intend the user to designate.
...
> Answer: The packages and any warnings show up.
...
* *dec =* : define *dec="."* or *dec=","* depending on how it's defined on your set. The default is ".". In many other countries, they use commas instead of decimal points. So when using data from places that use the comma then need to use the dec command so R knows to equate the two

```{r importdata}
# Absolute file path (not recommended b/c have to specify entire location)
#If you don't have the same folders as Luana b/c the absolute file path includes computer specific files.
...
# Relative file path (friendly for users regardless of machine. As long as the people have the same KEY folder with all the subfolders i.e. E_D_A_2022 then a relative path should work for everyone); set working directory then you can read data in from a relative path. just start from where the data deviates or goes deeper than the working directory
...
#you now need to add stringAsFactors = TRUE as an argument to the end of your dataset import so that characters read in as factors b/c this is no longer a default

#type the first dot and front slash and then R will give you options then you can use tab to find other folders that are deeper
#For knitting the .Rmd file you need to add an extra "." to the relative path before the first front slash
...
# What happens if we don't assign a name to our imported dataset? Then data is not stored in our environment aka doesn't show up in Environment tab on right and you can't work with the unstored data
#read.csv("../Data/Raw/USGS_Site02085000_Flow_Raw.csv")

# Another option is to choose with windows explorer/file finder
...
> ANSWER: In the Data Folder -> MetaData Folder. A README is a guide for others who may use the data to understand what the data is, where it came from, etc.

```{r exploredata}
View(USGS.flow.data)
# Alternate option: click on data frame in Environment tab
# click the white arrow in blue circle next to dataset in the environment to see the column names

#shows how data is shown. returns data.frame in this case. what we wanted.
class(USGS.flow.data)
#lets us see the headers
colnames(USGS.flow.data)

# Rename columns so they are informative, can use the README to know what the info is. C is short for column bind we are binding the info together
...
#shows dataframe and all objects in dataframe
str(USGS.flow.data)
#tells you how many rows and how many columns. Dim is short for dimensions.
dim(USGS.flow.data)
...
#insert number after the dataset in head function to specify how many rows to see
head(USGS.flow.data, 10)

#shows bottom of datset
tail(USGS.flow.data, 5)

#can use matrix dataframe to access certain sections of dataframe. Row 30000 to 30005 and columns 3,8, and 14
USGS.flow.data[30000:30005, c(3, 8, 14)]

#use this to figure out the type of data in each column
...
summary(USGS.flow.data)  #could point to a specific column only with $ followed by name of column

```

What happened to blank cells in the spreadsheet when they were imported into R?
> Answer: The blanks are filled in with NAs.
...
```{r findNA}
#returns summary stats including how many NAs there are
...
> Answer: NAs may need to be included if knowing the number of responses to different questions is important. Also, time series with NAs in the middle, NAs should be kept. A dataset that may not be important is when they are at the beginning or end of the dataset and outside of your timeframe. 

```{r removeNA}
#remove NAs from the data and assign the dataset to a new name so keep the old data with the NAs; keep the original data intact!!! Just removes rows nothing done to columns
USGS.flow.data.complete <- na.omit(USGS.flow.data)
dim(USGS.flow.data)
dim(USGS.flow.data.complete)

#examples of analysis where eliminating NAs doesn't matter
...
Tell R that column is dates and the format of the dates.
Capitalization matters!!
Formatting of dates in R: 

%d  day as number (0-31), lowercase d
%m  month as a number(00-12, can be e.g., 01 or 1), lowercase m
%y  2-digit year, lowercase y
%Y  4-digit year, uppercase Y
%a  abbreviated weekday, lowercase a
%A  unabbreviated weekday, uppercase A
%b  abbreviated month, lowercase b
%B  unabbreviated month, uppercase B
...
#first input in the as.Date is the column with the dates, original format of the date in data. the following code teaches R how to change the data in the Date column from a character class to a data class. R also changes the date to the international format of day, month, two-digit year. 
...
```{r fixformartDate}
# format changes the dates back to character class; which in this case removes the 20 in front of the early dates
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")

#Our last observation is Dec 19, 2019 i.e. the 191226 below. Paste0 glues strings and characters together. We are gluing 19 or 20 to d the input that is in our date/time column
create.early.dates <- (function(d) {
       paste0(ifelse(d > 191226,"19","20"),d)
       })

#This runs the above function on our data
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)

#return the characters back into dates; with a capital Y so see the 19 or 20
...
```{r savingdata}
#DO NOT include an extra . for the rmd like above
